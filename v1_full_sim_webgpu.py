...
shader_for = f"""
    ...
    fn main(@builtin(local_invocation_id) threadIdx: vec3<u32>,
        ...
        let z_src: i32 = z_sz/2;                // source term z position
...
def sim_webgpu_for(coef):
    ...
    # source term
    b4 = device.create_buffer_with_data(data=np.array(src_gpu[0]), usage=wgpu.BufferUsage.COPY_DST |
    ...
    for i in range(t_source):
        # Escreve o valor do termo de fonte a cada iteração
        device.queue.write_buffer(b4, 0, src[i])
        compute_pass.dispatch_workgroups(nx)  # x y z -- num blocks
    compute_pass.end()
    device.queue.submit([command_encoder.finish()])
    # Lê PK, o qual recebe o valor de termo de fonte
    # Ou seja, todos os valores da grid estão com o valor de tempo de fonte
    # referente ao tempo da última iteração (t_source-1)
    # Só para verificar se os dados foram escritos corretamente
...
# Compara o termo de fonte da CPU com a GPU
print(f'Termo de fonte CPU: {src[t_source-1]}')
print(f'Termo de fonte GPU: {u_for[0,0]}')
